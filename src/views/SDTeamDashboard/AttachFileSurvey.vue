      ...
      >
        ...
        <v-col cols="4" align-self="center" class="pa-1">
          <v-list-item lines="one">
            <v-list-item-title class="font-weight-black text-secondary">{{
              i.uploaded_email
            }}</v-list-item-title>
            <v-list-item-subtitle class="font-weight-medium text-black">{{
              dateUtils.parseDdMmYyWithTimeAndSeconds(i.uploaded_at)
        ...
        <v-col cols="2" align-self="center">
          <div class="text-center">
            <v-btn
              ...
              rounded
              size="small"
              @click="handleExport(i.name, i.type, i.url)"
            >
...
const getRspSurveyResultDocument = async (bp_number, rsp_suvery_id) => {
  try {
    loading.value = true;
    const response = await rspService.getRspSurveyResultDocument(
      bp_number,
      rsp_suvery_id
    );
