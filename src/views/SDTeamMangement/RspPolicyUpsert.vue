  ...
  <v-container>
    ...
    <div class="mt-5">
      <v-card class="elevation-1">
        <v-card-text>
          <div>
            ...
            <v-text-field v-model="name" variant="outlined" placeholder="ชื่อ policy" density="compact"></v-text-field>
          </div>
          <Choosefile @input-file="handleOnChangeFile" :is-file="file" title="อัพโหลด policy" list-title="uploaded Files"
      ...
      <div class="text-center mt-5">
        ...
        <v-btn rounded class="ma-2" color="secondary" style="width: 100px;" @click="handleAcceptConfirmed">
          <strong>ตกลง</strong>
        </v-btn>
      </div>
    </div>
...
const  handleConfirmedUpdated = async () =>{
  try {
      ...
      const v_file = file.value;
      const v_active = (selected.value ==='now') ? dateUtils.parseDdMmYyyy(new Date()) : selectedActiveDate.value;
      const response = await RspService.updateRspPolicy(v_id,v_name,v_file,v_active);
      if (response.data?.is_success) {
